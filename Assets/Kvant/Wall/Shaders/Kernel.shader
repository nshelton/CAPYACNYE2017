...
Shader "Hidden/Kvant/Wall/Kernel"
{
    ...
    float3 _NoiseOctaves;


    float snoiseFBM(float3 p)
    {
        float val = 0.0;
        val += snoise(p) * _NoiseOctaves.x; p *= 3;
        val += snoise(p) * _NoiseOctaves.y*0.5; p *= 3;
        val += snoise(p) * _NoiseOctaves.z*0.25; 
        return val;
    }
    ...
    float4 frag_position(v2f_img i) : SV_Target
    {
        ...
        // Displacement
    #if POSITION_Z
        float3 disp = float3(0, 0, snoiseFBM(nc));
    #elif POSITION_XYZ
        float3 disp = snoise_grad(nc) * 0.25;
    #else // POSITION_RANDOM
        float3 disp = get_rotation_axis(uv) * snoiseFBM(nc);
    #endif
    ...
    float4 frag_rotation(v2f_img i) : SV_Target
    {
        ...
        // Angle
        float angle = snoiseFBM(nc) * _RotationNoise.z;
    ...
    #if SCALE_UNIFORM
        float3 axes = (float3)snoiseFBM(nc + float3(417.1, 471.2, 0));
    #else // SCALE_XYZ
